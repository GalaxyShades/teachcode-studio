ALTER TABLE cms_lessons ADD COLUMN IF NOT EXISTS version integer NOT NULL DEFAULT 0;
ALTER TABLE cms_lessons ADD COLUMN IF NOT EXISTS position integer NOT NULL DEFAULT 0;
ALTER TABLE cms_lessons ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'draft';
ALTER TABLE cms_content_blocks ADD COLUMN IF NOT EXISTS advanced boolean NOT NULL DEFAULT false;
ALTER TABLE cms_mcq_options ADD COLUMN IF NOT EXISTS stable_id text;
ALTER TABLE cms_code_exercise_blocks ADD COLUMN IF NOT EXISTS expected_output text;
CREATE TABLE IF NOT EXISTS cms_revision_metadata (
 revision_id UUID PRIMARY KEY REFERENCES cms_lesson_revisions(id) ON DELETE CASCADE,
